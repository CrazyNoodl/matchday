...
| Playwright E2E tests (26 tests, 12 smoke) — includes match detail (score edit/commentary/delete), Reset All Data cooldown guardrail, and local backup/restore | `e2e/` — `npm run e2e`, `npm run e2e:smoke` |
...
### 8. "Maximum update depth exceeded" crash on browser back-nav into archive-day (demo mode) — newly found, not fixed

In demo mode (web), navigating away from `/archive-day` (e.g. to `/settings/display`) and then returning via the browser **back** button (not in-app nav) triggers a React "Maximum update depth exceeded" error, caught by the root ErrorBoundary ("Something went wrong"). Reproduces on plain `dev` with no other changes — found while e2e-verifying the groupByTours toggle (which itself works fine; the crash is unrelated and pre-exists this feature). Not yet root-caused — suspect an infinite `viewingRound`/`archivedRounds` derived-state loop in `app/archive-day.tsx` triggered specifically by back-navigation re-mounting with stale/mismatched store state. Forward in-app navigation (tapping a round card) does not trigger it.
