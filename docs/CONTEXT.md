...
**Fixed bug (2026-07-06):** tapping "Import from file" crashed the app on a physical device (worked fine on web). Root cause: `expo-document-picker` and the standalone `expo-file-system` were added to `package.json` when this feature was built, but `ios/`/`pod install` weren't regenerated afterward — the on-device Release build's native project never linked `ExpoDocumentPicker` at all, so `DocumentPicker.getDocumentAsync()` threw a native-module-not-found error with no try/catch around it. **Fix:** wrapped the `DocumentPicker.getDocumentAsync()` call in `pickAndReadBackupFile()` (`src/utils/backup.ts`) in try/catch, degrading to the existing `readError` UI state instead of crashing; ran `npx expo prebuild --clean` to relink the native modules and rebuilt the device Release build. Takeaway: adding a native Expo module to `package.json` requires a `prebuild`/`pod install` + fresh native rebuild before it'll work on a physical device — a Metro-only reload isn't enough.
